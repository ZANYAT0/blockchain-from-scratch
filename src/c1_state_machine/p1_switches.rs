...
use crate::c1_state_machine::p6_open_ended::State;
...
impl StateMachine for WeirdSwitchMachine {
    ...
    fn next_state(starting_state: &TwoSwitches, t: &Toggle) -> TwoSwitches {
        return match t {
            Toggle::FirstSwitch => {
                TwoSwitches { first_switch: !starting_state.first_switch, second_switch: false }
            }
            Toggle::SecondSwitch => {
                TwoSwitches { first_switch: starting_state.first_switch, second_switch: !starting_state.second_switch }
            }
        }
    }
}
